...
import { createBins, sortIntoBins } from "../utils/utils";
...
const HeatmapPitchChart: React.FC<HeatmapTooChartProps> = ({ pitches }) => {
  if (pitches.length == 0) {
    return null;
  }
  ...
  const firstColumnData = chartData.slice(0, 5);
  const secondColumnData = chartData.slice(5, 10);
...
export default HeatmapPitchChart;
