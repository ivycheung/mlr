...
const HistogramPitchChart: React.FC<HistogramPitchProps> = ({ pitches }) => {
  const [abResultOption, setAbResultOption] = React.useState<
    readonly AtBatResultData[]
  >([
  ...
  const handleAbChange = (dataToChange: AtBatResultData) => () => {
    setAbResultOption(
      abResultOption.map((d) =>
        d.key == dataToChange.key ? { ...d, status: !d.status } : d
      )
    );
  };
  ...
  const bucketSizeLabel = [{ value: 50, label: 50 }, { value: 100, label: 100 }, { value: 200, label: 200 }];
  ...
  const chartData = bins.map((count, index) => ({
    bucket: `${index * bucketSizeOption + 1}-${Math.min((index + 1) * bucketSizeOption, 1000)}`,
    count
  }));
      ...
      <Grid className="histogramChart">
        ...
        >
          <Box sx={{ textAlign: "center" }}>
            <Typography variant="h6">Histogram of Career Pitches</Typography>
          </Box>
          <Grid size={{ xs: 12 }}>
            <div>
