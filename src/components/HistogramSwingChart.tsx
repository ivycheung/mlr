  ...
  if (pitches.length !== 0) {
    ...
    const createBins = (pitches: FormSchemaPitches, bucketSize: number) => {
      ...
      pitches.forEach((pitch) => {
        if (pitch.swing >= 1 && pitch.swing <= 1000) {
          const index = Math.floor((pitch.swing - 1) / bucketSize); // Calculate which bucket this number falls into
          bins[index]++;
        }
      });
      return bins;
      ...
      <Grid className="histogramChart" >
        ...
        <Stack
          ...
          <Box sx={{ textAlign: 'center' }}>
            <Typography variant="h6">Histogram of Swings</Typography>
          </Box>
...
export default HistogramSwingChart;